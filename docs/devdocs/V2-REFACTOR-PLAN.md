...
| **A3 (done, allocation)** | Unify the peer-IP allocator across all four WG/AWG sites onto `net_next_free_octet` (config scan ∪ live-interface octets, max+1). Fixes the lib count+1 collision bug (26 WG/28 AWG dupes on a live server). `net-alloc-test.sh` unit + isolation harness on live configs. **Follow-up (A3b):** fold the per-path client-config *rendering* bodies (they still differ in MTU/instructions/QR + AWG obf-param source) into the libs — the −~340 ln. | WG/AWG triple-impl | e2e wireguard/wstunnel/amneziawg |
