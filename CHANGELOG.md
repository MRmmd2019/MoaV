...
### Fixed
- **`moav regenerate-users` could break WireGuard/AmneziaWG for users whose key material isn't in state.** `*_add_peer` handled two cases — state present (reuse) and no state + no peer (create) — but not the third: **no state while the server already has a peer**. It minted a fresh keypair and address, then the "peer already in config, skipping" guard declined to install them, so the regenerated bundle carried credentials the server had never seen and that user's tunnel stopped working. The private key only ever existed in the user's bundle, so it cannot be recovered. On a real 147-peer server only 36 of 146 users had `wireguard.env` (34 had `amneziawg.env`), so a single `moav regenerate-users` would have silently broken WireGuard for ~110 working users. The third state is now detected and **nothing is touched**: the existing peer and bundle are left exactly as they are, and the operator is told the user can only be re-issued via `moav user revoke` + `moav user add`. Verified against a copy of that server's config across all three states: peer+no-state returns 2 and mints nothing (server peer byte-identical afterwards), a brand-new user still allocates and appends normally, and an existing user with state is reused without duplicating the peer. Pre-existing since at least 1.9.1 — the v2 allocator work changed how the address is computed, not this branch.
